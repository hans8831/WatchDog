...
//
//
//  Created by Love Mob on 12/27/16.
...
class MainAlarmViewController: UITableViewController{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if Alarms.sharedInstance.count != 0
        {
            ...
            self.navigationItem.leftBarButtonItems?.append(backButtonItem)
...
//        UNUserNotificationCenter.current().removeAllPendingNotificationRequests()
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "AlarmCell", for: indexPath) as UITableViewCell
...
//        }
    ...
    @IBAction func backBtnClick(_ sender: UIBarButtonItem) {
        if(rootViewController != nil){
            appDelegate.backRootViewController()
        }
        else{
